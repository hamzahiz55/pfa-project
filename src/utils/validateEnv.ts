...
export function validateFileUpload(file: File): string | null {
  const maxSize = parseInt(process.env.MAX_FILE_SIZE || '52428800', 10); // 50MB default (increased from 5MB)
  const allowedTypes = (process.env.ALLOWED_FILE_TYPES || '*/*').split(','); // Allow all file types by default

  if (file.size > maxSize) {
    return `File size must be less than ${(maxSize / 1024 / 1024).toFixed(0)}MB`;
  }

  // If allowedTypes includes '*/*' or '*', allow all file types
  if (allowedTypes.includes('*/*') || allowedTypes.includes('*')) {
    return null; // All file types allowed
  }
